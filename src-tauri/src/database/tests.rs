...
fn schema_create_tables_repairs_legacy_proxy_config_singleton_to_per_app() {
    ...
    let count: i32 = conn
        ...
        .expect("count rows");
    assert_eq!(count, 5, "per-app proxy_config should have 5 rows (含 claude-cn)");
    ...
    let _: i32 = conn
        ...
        .expect("query claude row");
    let _: i32 = conn
        .query_row(
            "SELECT COUNT(*) FROM proxy_config WHERE app_type = 'claude-cn'",
            [],
            |r| r.get(0),
        )
        .expect("query claude-cn row");
}
