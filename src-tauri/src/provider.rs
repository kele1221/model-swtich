...
impl Provider {
    ...
    pub fn resolve_usage_credentials(
        ...
        let (base_url, api_key) = match app_type {
            ...
            // Claude / Claude CN / Claude Desktop all use the Anthropic-style env
            // map, keeping the OpenRouter/Google key fallbacks the JS-script path
            // relies on. Listed explicitly (not `_`) so a new AppType fails to
            // compile here.
