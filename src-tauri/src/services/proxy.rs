...
impl ProxyService {
    ...
    async fn rollback_hot_switch_preparation(
        ...
        let live_result = if matches!(app_type, AppType::Claude) {
            self.sync_claude_live_from_provider_while_proxy_active(&AppType::Claude, &previous_provider)
                .await
    ...
    async fn takeover_live_configs(&self) -> Result<(), String> {
        ...
        if let Ok(mut live_config) = self.read_claude_live() {
            ...
            let claude_provider = self.claude_provider_with_effective_settings(&AppType::Claude, &claude_provider)?;
            Self::apply_claude_takeover_fields_for_provider(
                &mut live_config,
                &proxy_url,
                &claude_provider,
            ...
            AppType::Claude => {
                if let Ok(mut live_config) = self.read_claude_live() {
                    ...
                    if let Some(provider) = claude_provider.as_ref() {
                        let provider = self.claude_provider_with_effective_settings(&AppType::Claude, provider)?;
                        Self::apply_claude_takeover_fields_for_provider(
                            &mut live_config,
                            &proxy_url,
                            &provider,
