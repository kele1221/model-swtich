...
impl McpService {
    ...
    fn sync_server_to_app_no_config(server: &McpServer, app: &AppType) -> Result<(), AppError> {
        match app {
            ...
            AppType::ClaudeCn => {
                log::debug!("Claude CN MCP not supported in first stage, skipping sync");
            }
    ...
    fn remove_server_from_app(_state: &AppState, id: &str, app: &AppType) -> Result<(), AppError> {
        match app {
            ...
            AppType::ClaudeCn => {
                log::debug!("Claude CN MCP not supported in first stage, skipping remove");
            }
            AppType::ClaudeDesktop => {
                log::debug!("Claude Desktop 3P profiles do not use CC Switch MCP sync, skipping");
            }
    ...
    fn project_servers_to_app(
        ...
        if matches!(app, AppType::ClaudeCn | AppType::OpenClaw | AppType::ClaudeDesktop) {
            return Ok(());
        }
