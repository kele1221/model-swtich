...
fn import_default_config_claude_persists_provider() {
    ...
    let db_path = home.join(".model-switch").join("model-switch.db");
    assert!(
        db_path.exists(),
        "importing default config should persist to model-switch.db"
    );
}
...
fn import_mcp_from_claude_creates_config_and_enables_servers() {
    ...
    assert!(
        db_path.exists(),
        "state.save should persist to model-switch.db when changes detected"
    );
}
